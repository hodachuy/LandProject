...
namespace LandProject.Common
{
	public class CommonSer
	{
       ...
       //                     }
		...
		public static string ConvertFilertFormLawToString(Filter filter)
		{
			StringBuilder _whereClause = null;
			if (filter != null && (filter.filters != null && filter.filters.Count > 0))
			{
				...
				for (var i = 0; i < filters.Count; i++)
				{
					...
					else
					{
						...
						{
							if (filters[i].Field == "IsExpired")// cột hiệu lực VBL
							{
								if (filters[i].Value == "1")
									_whereClause.Append("L.ExpirationDate < GETDATE()");
								else if (filters[i].Value == "0")
			...
			// Filter kết hợp với ngày từ - đến IssuedDate
			if (filter != null && (filter.filters != null && filter.filters.Count > 0))
			{
				var filters = filter.filters;
				...
				for (var i = 0; i < filters.Count; i++)
				{
					if (filter.filters[i].Field != null && filter.filters[i].Field.Contains("IssuedDate"))
					{
				...
				{
					filters.RemoveAll(x =>x.Field != "IssuedDate");
					if (filters.Count == 1)
					{
						if (filters[0].Operator == "gte")
						{
							_whereClause.Append(" AND (L.IssuedDate >= '" + ConvertTime(filters[0].Value) + " 00:00:00')");
						}
						else if (filters[0].Operator == "lte")
						{
							_whereClause.Append(" AND (L.IssuedDate <= '" + ConvertTime(filters[0].Value) + " 23:59:59')");
						}
					}
					else
					{
						...
						{
							_whereClause.Append(" AND (L.IssuedDate >= '" + ConvertTime(filters[0].Value) + " 00:00:00' AND L.IssuedDate <= '" + ConvertTime(filters[1].Value) + " 23:59:59')");
						}
					}
